...
#Route to root
@app.route("/") #I assume that the app() program is constantly running and reading what the user enters which then dictates the execution of these function....surprised that beyond this "app" program, there isn't another brain child out here doing stuff. Default handling is a GET request...no need to specify as default is GET. Any other request will be ignored. 
def show_list():
    db = get_db()
    cur = db.execute('SELECT what_to_do, due_date, status FROM entries')
    entries = cur.fetchall() #returns object as a cursor (which is/should be a list)
